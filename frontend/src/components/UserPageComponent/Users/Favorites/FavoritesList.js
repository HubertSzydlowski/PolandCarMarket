...
const UserFavoritesList = () => {
    ...
    const handleAdd = async () => {
        ...
        if (!id) return;
        try {
            await UserPageFavouritesService.addToFavorites(id);
            setNewAdId('');
            await fetchFavorites();
        } catch (err) {
            console.error(err);
    ...
    const openOffer = (adId) => window.open(`/offers/${adId}`, "_blank");
                    ...
                    {allData.map(({ advertisement, vehicle }) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <button
                                    onClick={(e) => { e.stopPropagation(); handleRemove(advertisement.id); }}
                                ...
                                >
                                    <img src="/icons/trash.png" alt="delete icon" style={{ width: 16, height: 16}} />
                                </button>
                            </li>
                        );
                    })}
