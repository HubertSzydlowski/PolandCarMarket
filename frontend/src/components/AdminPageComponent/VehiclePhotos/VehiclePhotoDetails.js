...
import adminPhotoService from '../../../services/AdminPageServices/AdminPageVehiclePhotoService';

function AdminVehiclePhotoDetails() {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchPhoto = async () => {
            try {
                const data = await adminPhotoService.getPhotoById(id);
                setPhoto(data);
            } catch (error) {
                console.error('Error fetching photo (admin):', error);
            }
        };
...
export default AdminVehiclePhotoDetails;
