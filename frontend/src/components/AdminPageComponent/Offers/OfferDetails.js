...
const AdminOfferDetails = () => {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                console.error(e);
                if (mounted) setError(e.message || "Error loading admin offer");
            } finally {
    ...
    const copyId = async () => {
        try { await navigator.clipboard.writeText(String(advertisement.id)); alert("ID copied!"); }
        catch { alert("Failed to copy ID"); }
    };

    const copyPhone = async () => {
        try { await navigator.clipboard.writeText(String(advertisement.phoneNumber)); alert("Phone number copied!"); }
    ...
    return (
        <>
            <div style={{ width: "100%", boxSizing: "border-box", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 24px", backgroundColor: "#8b0000", color: "white", position: "sticky", top: 0, zIndex: 10 }}>
                ...
                <button style={{ padding: "8px 16px", fontWeight: "bold", borderRadius: "6px", border: "none", cursor: "pointer", backgroundColor: "white", color: "#8b0000", display: "flex", alignItems: "center", gap: 10 }} onClick={() => navigate("/my-account")}>
                    My account <img src="/icons/user.png" alt="user icon" style={{ width: 20, height: 20 }} />
                </button>
            </div>
            ...
            <div style={{ padding: "0 16px 16px 16px", maxWidth: 1100, margin: "0 auto" }}>
                ...
                <div style={{ marginTop: 20 }}>
                    <Link to={`/admin/offers/${id}/edit`} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "10px 20px", backgroundColor: "#8B0000FF", color: "white", textDecoration: "none", borderRadius: "4px", fontWeight: "bold" }}>
                        <img src="/icons/edit.png" alt="Edit" style={{ width: 18, height: 18 }} />
                        Edit offer
                    </Link>
                </div>
                ...
                <header style={{ margin: "16px 0 12px 0", borderRadius: 8, overflow: "hidden" }}>
                    ...
                    <div style={{ backgroundColor: "#fff", padding: 12, display: "flex", gap: 8, alignItems: "center", border: "1px solid #e6e6e6" }}>
                        ...
                        <button onClick={copyId} style={{ backgroundColor: '#8b0000', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: 6, cursor: 'pointer', fontWeight: 600, display: "flex", alignItems: "center", gap: 4 }}>
                            <img src="/icons/copy.png" alt="Copy" style={{ width: 16, height: 16 }} /> Copy ID
                        </button>
                    </div>
                </header>
                    ...
                    <aside style={{ width: 320, minWidth: 240 }}>
                        <div style={{ borderRadius: 8, overflow: "hidden", boxShadow: "0 6px 18px rgba(0,0,0,0.04)" }}>
                            <div style={{ backgroundColor: "#8b0000", color: "#fff", padding: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 8 }}>
                                <img src="/icons/car.png" alt="Car" style={{ width: 20, height: 20 }} /> Vehicle details
                            </div>
                ...
                {vehicle?.description && (
                    <section style={{ marginTop: 20, borderRadius: 8, overflow: "hidden" }}>
                        <div style={{ backgroundColor: "#8b0000", color: "#fff", padding: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 8 }}>
                            <img src="/icons/description.png" alt="Description" style={{ width: 20, height: 20 }} /> Vehicle description
                        </div>
                        <div style={{ backgroundColor: "#fff", padding: 12 }}>
                ...
                {advertisement.googlePlaceId && (
                    <section style={{ marginTop: 20, borderRadius: 8, overflow: "hidden" }}>
                        <div style={{ backgroundColor: "#8b0000", color: "#fff", padding: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 8 }}>
                            <img src="/icons/location.png" alt="Location" style={{ width: 20, height: 20 }} /> Location
                        </div>
