...
const AdminOfferList = () => {
    ...
    const loadAllOffers = async () => {
        ...
        try {
            ...
            console.error(err);
            setError("Error loading admin offers");
        } finally {
            setLoading(false);
        }
    };
    ...
    const openDetailsInNewTab = (id) => { window.open(`/admin/offers/${id}`, "_blank"); };
    ...
    const handleDeleteOffer = async (e, id) => {
        e.stopPropagation();
        if (!window.confirm("Are you sure you want to delete this advertisement (admin)?")) return;
        try {
            if (!tokenManager.canAccessAdminPage()) {
                alert("No permissions to delete advertisement.");
                return;
            }
            await AdminPageOfferService.deleteOffer(id);
            setAllOffersRaw((prev) => prev.filter(item => String(item.advertisement.id) !== String(id)));
            alert("Advertisement deleted (admin).");
        } catch (err) {
    ...
    const handleEdit = (e, id) => {
        e.stopPropagation();
        window.open(`/admin/offers/${id}/edit`, "_blank");
    };
        ...
        <div style={{ boxSizing: "border-box" }}>
            ...
            <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 20px", boxSizing: "border-box" }}>
                ...
                <div style={{ marginBottom: 20 }}>
                    <button
                        type="button"
                        onClick={() => window.open("/admin/offers/create", "_blank")}
                        style={{
                            display: "inline-flex",
                            alignItems: "center",
                            gap: 6,
