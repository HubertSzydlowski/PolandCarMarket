...
const OfferCreateForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!title.trim()) {
            setError("Tytuł ogłoszenia jest wymagany.");
            return;
        }
        if (!userId || isNaN(Number(userId))) {
            setError("Poprawne userId (liczba) jest wymagane.");
            return;
        }
        if (!uploader || !uploader.trim()) {
            setError("Uploader (username) musi być podany ręcznie.");
            return;
        }
        ...
        try {
            ...
            if (adId) {
                navigate(`/offers/${adId}`);
            } else {
                navigate("/offers");
            }
        } catch (err) {
            console.error("Create offer error:", err);
            setError(err?.message || "Błąd przy tworzeniu oferty");
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <div style={{ backgroundColor: "#fff", padding: 16 }}>
                        <form onSubmit={handleSubmit}>
                                    ...
                                    <div style={{ position: "relative", display: "inline-block" }}>
                                        ...
                                        {showTooltip && (
                                            ...
                                            >
                                                ...
                                                Example:
                                                <div style={{ marginTop: 4, fontSize: 10, color: "#ccc", wordBreak: "break-all" }}>
                                                    https://www.google.pl/maps/place/53%C2%B009'54.5%22N+18%C2%B009'11.4%22E/@53.1623397,18.1564397,14z/data=!4m4!3m3!8m2!3d53.165144!4d18.153178?hl=pl&entry=ttu&g_ep=EgoyMDI1MDkxMC4wIKXMDSoASAFQAw%3D%3D
                                                </div>
                                            </div>
                                        )}
                                    </div>
                            ...
                            <div style={{ marginBottom: 12, display: "flex", flexDirection: "column", position: "relative" }}>
                                ...
                                <input type="file" multiple accept="image/*" onChange={handleFilesChange} style={{ marginTop: 4 }} />
