...
const OfferList = () => {
    ...
    const [userIsSimpleUser, setUserIsSimpleUser] = useState(false);
    ...
    const [filters, setFilters] = useState({
        brand: "", model: "", minYear: "", maxYear: "", minPrice: "", maxPrice: "", minMileage: "", maxMileage: "",
    ...
    const loadAllOffers = async () => {
        ...
        try {
            ...
            while (pageNum < totalPages) {
                const res = await fetch(`${BASE}/advertisements?page=${pageNum}&size=${pageSize}`, {
                    credentials: "include",
                    headers: { "Content-Type": "application/json", ...authHeaders() },
                });
            ...
            const promises = ads.map(async (ad) => {
                ...
                if (vehicle && vehicle.id) {
                    ...
                    return {
                        ...
                        vehicle: {
                            ...vehicle,
                            photos: photos.map((p) => ({ ...p, fullPhotoUrl: buildFullPhotoUrl(p) })),
                            mainPhoto: mainPhoto ? { ...mainPhoto, fullPhotoUrl: buildFullPhotoUrl(mainPhoto) } : null,
                        },
                    };
                }
                return { advertisement: ad, vehicle: null };
            });
    ...
    useEffect(() => {
        ...
        const rolesArray = (payload && (payload.roles || payload.authorities || payload.role || payload.rolesArray)) || null;

        const hasUserRole = (() => {
            if (!rolesArray) return false;
            if (Array.isArray(rolesArray)) {
                return rolesArray.some(r => {
                    const val = typeof r === "string" ? r : (r.authority || r.role || JSON.stringify(r));
                    return String(val).toUpperCase().includes("USER");
                });
            }
            if (typeof rolesArray === "string") return rolesArray.toUpperCase().includes("USER");
            return false;
        })();

        setUserIsSimpleUser(hasUserRole);

        if (hasUserRole) {
            (async () => {
                setFavsLoading(true);
                try {
                    const favData = await UserPageFavouritesService.getFavorites();
                    const ids = (Array.isArray(favData) ? favData : (favData?.content || []))
                        .map(item => (typeof item === 'object' ? (item.id ?? item.advertisementId ?? item) : item))
                        .filter(Boolean);
    ...
    const toggleFavorite = async (e, adId) => {
        e.stopPropagation();
        if (!userIsSimpleUser) {
            alert("You must be logged in as a regular user to manage favorites.");
            return;
        }
        const idStr = String(adId);
        const already = favoriteIds.has(idStr);

        try {
            if (already) {
                await UserPageFavouritesService.removeFromFavorites(adId);
                const newSet = new Set([...favoriteIds].filter(x => x !== idStr));
                setFavoriteIds(newSet);
            } else {
                await UserPageFavouritesService.addToFavorites(adId);
    ...
    const filteredSortedOffers = useMemo(() => {
        ...
        arr = arr.filter(item => {
            ...
            if (f.minYear && Number(v.year) < Number(f.minYear)) return false;
            if (f.maxYear && Number(v.year) > Number(f.maxYear)) return false;
            ...
            if (f.warranty) {
                const hasWarranty = v.hasWarranty ? "yes" : "no";
                if (hasWarranty !== f.warranty) return false;
            }
            return true;
        });
    ...
    const openDetailsInNewTab = (id) => { window.open(`/offers/${id}`, "_blank"); };
    ...
    const formatBrand = (rawBrand) => {
        ...
        return rawBrand.replace(/_/g, " ").split(" ").map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(" ");
    };
    ...
    const formatModel = (rawModel, rawBrand) => {
        ...
        if (brandKey && Array.isArray(brandModels[brandKey])) {
            ...
            if (foundModel) return foundModel.replace(/_/g, " ");
        }
        return rawModel.replace(/_/g, " ").split(" ").map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(" ");
    };
            ...
            <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 20px", boxSizing: "border-box" }}>
                            ...
                            <div style={inner}>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
                                    ...
                                    <label style={labelStyle}>
                                        ...
                                        >
                                            <option value="">--</option>
                                            {filters.brand && brandModels[filters.brand].map(m => <option key={m} value={m}>{m.replace(/_/g, " ")}</option>)}
                                        </select>
                                    </label>
                                </div>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "18px", boxSizing: "border-box" }}>
                    {pagedOffers.map(({ advertisement, vehicle }) => {
                        ...
                        const adIdStr = String(advertisement.id);
                        const isFav = favoriteIds.has(adIdStr);
                        return (
                            <li
                                ...
                                <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
                                    {userIsSimpleUser && (
                                        <button
                                            ...
                                            onClick={(e) => toggleFavorite(e, advertisement.id)}
                                        ...
                                        >
                                            <img
                                                ...
                                                style={{
                                                    width: 16,
                                                    height: 16,
                                                    opacity: isFav ? 1 : 0.65,
                                                    transform: isFav ? "scale(1.5)" : "scale(1)",
